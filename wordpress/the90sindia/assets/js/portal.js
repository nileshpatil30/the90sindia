...
(function () {
  ...
  /* Where the main hub lives. WordPress prints REWIND_HOME_URL ahead of this
     script; on the static site nothing defines it and the relative filename
     is correct, so one file serves both. */
  const HOME = (typeof REWIND_HOME_URL === "string" && REWIND_HOME_URL)
    ? REWIND_HOME_URL
    : "index.html";
  ...
  function buildCardModel() {
    ...
    if (SHOWS[0]) {
      cards.push({
        section: "SHOWS", icon: "📺", more: HOME + "#shows",
        title: SHOWS[0].title, blurb: SHOWS[0].desc, entry: SHOWS[0],
      });
    }
    if (CHANNELS[0]) {
      cards.push({
        section: "CHANNELS", icon: "📡", more: HOME + "#channel-guide",
        title: CHANNELS[0].name, blurb: CHANNELS[0].desc, entry: CHANNELS[0],
      });
    }
    cards.push({
      section: "ARCADE", icon: "🕹️", more: HOME + "#arcade",
    ...
    if (TRACKS[0]) {
      cards.push({
        section: "HIT PARADE", icon: "🎵", more: HOME + "#hit-parade",
